#!/bin/bash
#
# CVAT Multiview 초기 설정 및 Task 생성 스크립트
#
...
#   ./setup_and_create_tasks.sh --skip-superuser  # superuser 이미 있는 경우
#   ./setup_and_create_tasks.sh --dry-run         # Task 생성 미리보기
#
# 환경변수로 설정 가능:
#   CVAT_USER, CVAT_PASSWORD, CVAT_ORG, CVAT_HOST, DATA_DIR
#
...
환경변수:
  ...
  DATA_DIR            데이터셋 루트 경로 (기본값: /mnt/data)
...
create_organization() {
    ...
    if echo "$org_response" | grep -q "\"slug\":\"$CVAT_ORG\""; then
        ...
        log_warn "Organization '$CVAT_ORG'가 이미 존재합니다. 계속 진행합니다."
    else
...
# Step 3: Task 생성
create_tasks() {
    log_step "Step 3: Multiview Task 생성"
    ...
    log_info "create_all_tasks.sh 실행 중..."

    "$SCRIPT_DIR/create_all_tasks.sh" \
        --user "$CVAT_USER" \
...
main() {
    echo -e "${CYAN}"
    echo "============================================================"
    echo "  CVAT Multiview 초기 설정 및 Task 생성"
    echo "============================================================"
    echo -e "${NC}"
    ...
    log_info "Organization '$CVAT_ORG'의 멤버를 초대하려면:"
    log_info "  1. $CVAT_HOST 접속"
    log_info "  2. 좌측 메뉴 → Organizations → $CVAT_ORG"
    log_info "  3. Members → Invite"
}
