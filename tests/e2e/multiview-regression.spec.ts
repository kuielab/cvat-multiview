...
import { STORAGE_STATE, openMultiviewJob, openMultiviewJobWithShapes, clickViewButton } from './helpers';
...
test.describe('Regression Tests', () => {
    ...
    test('sidebar items count matches active view shapes', async ({ page }) => {
        await openMultiviewJobWithShapes(page);

        // Sidebar shows view-filtered items (only current view's annotations)
        // Canvas may show shapes from all views (preview + active overlays)
        // So we compare sidebar count with active canvas shapes only
        const activeShapeCount = await page.locator(
            '.annotation-canvas-overlay.active-canvas .cvat_canvas_shape',
        ).count();
        ...
        console.log(`Active canvas shapes: ${activeShapeCount}, Sidebar items: ${sidebarCount}`);
        // Sidebar count should match active view's shape count
        expect(sidebarCount).toBeGreaterThan(0);
        expect(activeShapeCount).toBeGreaterThan(0);
    });
